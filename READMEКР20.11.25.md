...
1. Коэффициент аппроксимации α для задачи минимизации — это число α ≥ 1, которое характеризует, во сколько раз решение, найденное приближённым алгоритмом, может быть хуже оптимального решения в худшем случае.
Для задачи минимизации, если:
- OPT — стоимость оптимального решения
- A(I) — стоимость решения, найденного алгоритмом A на входе I
Тогда алгоритм A имеет коэффициент аппроксимации α, если для всех входов I выполняется:
`text
A(I) ≤ α ⋅ OPT`
2. Что он характеризует:
- Качество приближения — насколько "плохим" может быть решение алгоритма по сравнению с идеальным
- Гарантированную границу — худший случай, а не средний
- Эффективность алгоритма — чем ближе α к 1, тем лучше алгоритм
3. Коэффициент аппроксимации позволяет:
- Сравнивать эффективность различных приближённых алгоритмов
- Гарантировать качество решения даже без знания OPT
- Выбирать алгоритмы для конкретных приложений
